...
# 1. 检查 config.ts 配置
echo "📋 1. 检查小程序配置..."
CONFIG_FILE="frontend/packages/miniprogram/utils/config.ts"
if [ -f "$CONFIG_FILE" ]; then
    if grep -q "http://127.0.0.1:5200" "$CONFIG_FILE"; then
        ...
        echo ""
        echo "      可选修复（切换到 dev 环境）:"
        echo "      sed -i \"\" \"s|export const API_BASE = .*|export const API_BASE = 'http://127.0.0.1:5200' // dev 环境|g\" $CONFIG_FILE"
    fi
else
    echo "   ❌ 找不到 config.ts，路径应为: $CONFIG_FILE"
fi
...
echo "   D. 工具 → 构建 npm"
  ...
  echo "      2. pages/ask/index (提问 + Consent: ask_rag)"
  echo "      3. app.ts / pages/splash (登录: wechatLogin)"
  echo "      4. pages/result/index (分享 + Consent: profile_share)"
  echo ""
...
echo "   pnpm run build:npm"
echo ""
echo "   # API 层验证（返回仓库根目录）"
echo "   cd ../../../.. && bash scripts/verify-p0-local.sh"
echo ""
echo "   # Web E2E 测试（参考）"
echo "   cd ../../.. && pnpm e2e:live"
echo ""
