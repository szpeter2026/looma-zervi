...
用法:
  python scripts/concurrency_test.py --url http://127.0.0.1:8010/v1/ask \
...
def run_load_test(url: str, auth_token: str, concurrency: int, total_requests: int, ready_url: str | None) -> None:
    if ready_url:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--url", default="http://127.0.0.1:8010/v1/ask", help="目标 /v1/ask URL")
    parser.add_argument("--token", default="token-b658c985", help="Bearer token")
    parser.add_argument("--concurrency", type=int, default=5, help="并发线程数")
    parser.add_argument("--requests", type=int, default=20, help="总请求数")
    parser.add_argument("--ready-url", default="http://127.0.0.1:8010/v1/health", help="服务就绪检测 URL")
    return parser.parse_args()


def main() -> int:
    args = parse_args()
    run_load_test(args.url, args.token, args.concurrency, args.requests, args.ready_url)
    return 0
